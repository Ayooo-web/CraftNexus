...
pub enum DataKey {
    ...
    /// Formerly `VerificationHistory`.
    VerifHistLegacy(Address),
    /// Count of compact verification history entries per user (#519).
    /// Formerly `VerificationHistoryCount`.
    VerifHistCount(Address),
    /// Indexed compact verification history entry (#519).
    /// Formerly `VerificationHistoryIndexed`.
    VerifHistEntry(Address, u32),
    /// Username change fee (in stroops) - Issue #114.
    /// Formerly `UsernameChangeFee`.
    UsernameFee,
    /// Token used to collect username change fees (#134).
    /// Formerly `UsernameChangeFeeToken`.
    UsernameFeeToken,
    /// Destination wallet for username change fees (#134).
    /// Formerly `UsernameChangeFeeWallet`.
    UsernameFeeWallet,
    /// Timestamp of last username change per user - Issue #114.
    /// Formerly `LastUsernameChange`.
    LastUNameChange(Address),
}
